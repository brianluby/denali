...
def _run(tmp_path: Path, *arguments: str, **overrides: str) -> subprocess.CompletedProcess[str]:
...
def test_production_deploy_rejects_non_main_workflow_source(tmp_path: Path) -> None:
    result = _run(tmp_path, "--confirm-production", GITHUB_REF="refs/heads/feature")
...
def test_production_deploy_requires_all_p0_provider_configuration(tmp_path: Path) -> None:
    result = _run(tmp_path, "--confirm-production")
